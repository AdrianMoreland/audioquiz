...
public class FrequencyStatsService implements FrequencyStatsApi {
    ...
    @Override
    public Observable<FrequencyStats> observeFrequencyStats() {
        return Observable.<FrequencyStats>create(emitter -> {
                    // Fetch the snapshot (single snapshot request)
                    firestoreDataSource.getDocumentSnapshot(
                                    USER_DATA_COLLECTION,
                                    FREQUENCY_STATS_DOCUMENT)
                            .addOnCompleteListener(task -> {
                                if (task.isSuccessful() && task.getResult() != null) {
                                    DocumentSnapshot documentSnapshot = task.getResult();
                                    handleDocumentSnapshot(documentSnapshot, (SingleEmitter<FrequencyStats>) emitter);
                                } else {
                                    emitter.onError(task.getException()); // Handle error
                                }
                            });
                })
                .subscribeOn(Schedulers.io())
